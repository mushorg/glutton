...
func (g *Glutton) Init() error {
	var err error
	g.publicAddrs, err = getNonLoopbackIPs(viper.GetString("network.interface"))
	if err != nil {
		return err
	}
...
func (g *Glutton) Start() error {
	...
	if err := setTProxyIPTables(viper.GetString("network.interface"), g.publicAddrs[0].String(), "tcp", uint32(g.Server.tcpPort), sshPort); err != nil {
		return err
	}

	if err := setTProxyIPTables(viper.GetString("network.interface"), g.publicAddrs[0].String(), "udp", uint32(g.Server.udpPort), sshPort); err != nil {
		return err
	}
...
func (g *Glutton) Shutdown() {
	...
	if err := flushTProxyIPTables(viper.GetString("network.interface"), g.publicAddrs[0].String(), "tcp", uint32(g.Server.tcpPort), uint32(viper.GetInt("ports.ssh"))); err != nil {
		g.Logger.Error("Failed to drop tcp iptables", producer.ErrAttr(err))
	}
	g.Logger.Info("Flushing UDP iptables")
	if err := flushTProxyIPTables(viper.GetString("network.interface"), g.publicAddrs[0].String(), "udp", uint32(g.Server.udpPort), uint32(viper.GetInt("ports.ssh"))); err != nil {
